...
mod_hm10_ui_update <- function(id) {
  # 1. Namespace
  ns <- NS(id)

  # 2. Series selection ----
  hm10_class_2 <- filter_series_unlist(
    guide,
    column = "Category_2",
    apply_filters = list(Dataset = c("hm10"))
  )
...
mod_hm10_server_update <- function(id, selected_tab, activate_on) {
  moduleServer(id, function(input, output, session) {
    enabled <- reactive(identical(selected_tab(), activate_on))
