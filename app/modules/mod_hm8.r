...
mod_hm8_ui_update <- function(id) {
  # 1. Namespace
  ns <- NS(id)

  # 2. Series selection ----
  hm8_split <- filter_series_unlist(
    guide,
    column = "Category_2",
    apply_filters = list(Dataset = c("hm8"))
  )
  hm8_split <- hm8_split[!is.na(hm8_split)]

  hm8_grouping <- filter_series_unlist(
    guide,
    column = "Category_1",
    apply_filters = list(Dataset = c("hm8"))
  )
...
mod_hm8_server_update <- function(id, selected_tab, activate_on) {
  moduleServer(id, function(input, output, session) {
    enabled <- reactive(identical(selected_tab(), activate_on))
