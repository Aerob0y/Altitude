...
.cache$paths <- list(
  series = list(specific = "", detectDates = FALSE, sheet = "Series Definitions", startRow = 1, skipEmptyRows = TRUE, files = c("reference/RBNZ_Series.xlsx")),
  ...
  hc35 = list(specific = "RBNZ", detectDates = TRUE, sheet = "Data", startRow = 5, skipEmptyRows = TRUE, files = c(latest_file(dir = "data/RBNZ", pattern = "^hc35-\\d{8}\\.xlsx$"))),
...
.is_rds_fresh <- function(name) {
  ...
  rds_file <- paste0(dirname(sources[1]), "/", name, ".rds")
...
.load_data <- function(name, return = TRUE, refresh = "Auto", specific = "") {
  sources <- unlist(.cache$paths[[name]]["files"])
  rds_file <- paste0(dirname(sources[1]), "/", name, ".rds")
