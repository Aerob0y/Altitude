download_latest_ect <- function(
  ...
  files <- fs::dir_ls("data/ECT/", regexp = "csv-tables.csv", type = "file")
  ...
  saveRDS(ect_values_division, file = "data/ECT/ect_values_division.rds", compress = FALSE)
  ect_values_industry <- data %>%
    filter(Group == "Values - Electronic card transactions A/S/T by industry group")
  saveRDS(ect_values_industry, file = "data/ECT/ect_values_industry.rds", compress = FALSE)
  ect_number <- data %>%
    filter(Group == "Number of electronic card transactions A/S/T by division")
  saveRDS(ect_number, file = "data/ECT/ect_number.rds", compress = FALSE)
  ect_mean <- data %>%
    filter(Group == "Electronic card transactions by mean and proportion")
  saveRDS(ect_mean, file = "data/ECT/ect_mean.rds", compress = FALSE)
  ect_totals_percentage_changes <- data %>%
    filter(Group == "Totals - Electronic card transactions by division, percentage changes")
  saveRDS(ect_totals_percentage_changes, file = "data/ECT/ect_totals_percentage_changes.rds", compress = FALSE)
