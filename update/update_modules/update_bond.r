...
update_bond_data <- function(only_rds = FALSE) {
  if (!only_rds) {
    if (dir.exists("app/data/Bond")) {
      data <- download_if_updated_etag("https://www.tenancy.govt.nz/assets/Uploads/Tenancy/Rental-bond-data/Detailed-Monthly-TLA-Tenancy-v2.csv", "app/data/Bond/Bond.csv")
      print("Folder exists")
    }
  }
