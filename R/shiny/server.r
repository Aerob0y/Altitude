...
server <- function(input, output) {
  ...
  output$hm3_plotx <- renderPlotly({
    ...
    rbnz_plotly(title = "Investment", subtitle = paste("RBNZ:", paste(x, collapse = " and ")), g = "hm3", split = x)
  })
  ...
  output$hm5_plotx <- renderPlotly({
    ...
    #}
    rbnz_plotly(title = "GDP", subtitle = "Subtitle", g = "hm5", names = input$hm5_names, names_fallback = 'Export of Goods & Services (Nominal $m)')
  })
  output$hm6_plotx <- renderPlotly({
    rbnz_plotly(title = "National Saving", subtitle = "Subtitle", g = "hm6", names = input$hm6_names)
  })
  output$hm7_plotx <- renderPlotly({
    rbnz_plotly(title = "Balance of Payments", subtitle = "Subtitle", g = "hm7")
  })
  output$hm8_plotx <- renderPlotly({
    rbnz_plotly(title = "Overseas Trade", subtitle = "Subtitle", g = "hm8", split = input$hm8_split, adj = input$hm8_adj)
  })
  ...
  output$hm10_plotx <- renderPlotly({
    rbnz_plotly(title = "Housing", subtitle = "Subtitle", g = "hm10", split = c(input$hm10_split_1, input$hm10_split_2)) 
  })
  ...
  output$hs32_plotx <- renderPlotly({
    rbnz_plotly(title = "Loans", subtitle = "Lending by group", g = "hs32", group = input$hs32_group, split = input$hs32_split, adj = input$hs32_adj)
  })
  output$hc35_plotx <- renderPlotly({
    rbnz_plotly(title = "Residential mortgage loan reconciliation", subtitle = "Lending by group", g = "hc35", group = input$hc35_group, split = input$hc35_split, adj = input$hc35_adj)
  })
